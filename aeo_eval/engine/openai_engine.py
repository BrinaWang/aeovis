"""OpenAI API engine implementation for the AEO Visibility Platform."""
...
class OpenAIEngine(BaseEngine):
    """AI engine that uses OpenAI's API."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not api_key:
            raise ValueError(
                "OPENAI_API_KEY required. Set via config or OPENAI_API_KEY env var."
            )
        ...
        logger.info(
            f"Initialized OpenAIEngine with model={self.model_name}, "
    ...
    def run(self, prompt_text: str) -> RunResult:
        """
        Run a prompt through OpenAI and return the result.
        ...
        try:
            ...
            logger.debug(f"Calling OpenAI API (model={self.model_name})")
            response = self._retry_manager.retry(_call_openai)
            ...
            logger.info(
                f"OpenAI API success: {input_tokens} input + {output_tokens} output tokens, "
    ...
    def get_token_costs(self) -> Dict[str, float]:
        """
        Get current token costs for gpt-4o.
        ...
        """
        return {
            "input": self.config.get("cost_per_1k_input_tokens", 0.0025),
            "output": self.config.get("cost_per_1k_output_tokens", 0.010),
        }
